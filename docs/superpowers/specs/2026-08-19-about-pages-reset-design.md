...
- 冶金事业部（`aboutDepartment === 'metallurgy'`）：复制参考项目矿山事业部页面的结构、统计、专家、技术方向、服务领域和代表性工程列表；由于当前没有冶金材料，只替换身份性标题为“冶金事业部”，不把矿山事实改写成冶金业绩。
...
- 冶金事业部：`ABOUT / 03`，Hero 身份文案为“长沙有色院 · 冶金事业部”，主标题为“冶金事业部”。
...
- 冶金事业部沿用矿山素材：`about/2/` 下的矿山图片、专家肖像和宣传册图片。
...
- 在当前 Met 项目中将事业部分支映射为 `metallurgy`，并将矿山页面的标题身份替换为冶金事业部；不新增 `mining` 路由。
...
4. 冶金事业部页面使用矿山页内容与素材，但页面身份标题明确显示“冶金事业部”；页面正文不新增未经提供的冶金工程事实。
