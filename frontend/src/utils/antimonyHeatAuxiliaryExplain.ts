...
import {
  ...
  type AntimonyHeatAuxiliaryParams,
  type AntimonyHeatAuxiliaryTrace,
  type OxygenEnrichmentGasTrace,
} from './antimonyHeatAuxiliaryParams.ts'
...
export const HEAT_AUXILIARY_EXPLAIN_ITEMS: HeatAuxiliaryExplainItem[] = [
  ...
  },
  {
    ...
    label: '熔炼总尘率',
    unit: '%',
    source: '产出「烟尘」总质量 ÷ 入炉「混合锑精矿」干基质量',
    formula: '烟尘质量 / 混合锑精矿质量 × 100',
    detail: '烟尘占混合锑精矿的百分数，对应 MetCal 侧吹熔炼炉辅助公式中的总尘率。',
    aliases: ['熔炼总尘率', '总尘率'],
  },
  {
  ...
  },
  {
    ...
    label: '机械尘',
    unit: '%',
    source: '产出烟尘物相中扣除 PbO、ZnO、As₂O₃ 后的质量 ÷ 混合锑精矿干基',
    formula: '(烟尘 − PbO − ZnO − As₂O₃) / 混合锑精矿 × 100',
    detail:
...
function formatHeatAuxiliaryWalkthrough(
  ...
  trace: AntimonyHeatAuxiliaryTrace | null | undefined
): string[] {
  if (!trace) return []
  ...
  switch (item.key) {
    ...
    case 'totalDustRatePct': {
      if (trace.dustMassTh != null && trace.concentrateMassTh > 0) {
        ...
        lines.push(`  烟尘质量 = ${formatAuxiliaryParam(trace.dustMassTh, 4)} t/h`)
        lines.push(`  混合锑精矿 = ${formatAuxiliaryParam(trace.concentrateMassTh, 3)} t/h`)
        lines.push(
...
export function formatHeatAuxiliaryExplainBlock(
  ...
  trace?: AntimonyHeatAuxiliaryTrace | null
): string {
  const lines = [
...
export function buildHeatAuxiliaryExplainReply(params?: {
  ...
  live?: AntimonyHeatAuxiliaryParams | null
  trace?: AntimonyHeatAuxiliaryTrace | null
}): string {
