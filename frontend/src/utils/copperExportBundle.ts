export type CopperExportBundleFile = {
  fileName: string
  content: ArrayBuffer
}

export type CopperExportBundleResult =
...
type ElectronExportBundleApi = {
  exportBundleToDirectory?: (
    folderName: string,
    files: CopperExportBundleFile[]
  ) => Promise<CopperExportBundleResult>
}

const DEFAULT_FLO_TEMPLATE_PATH = 'templates/southwest-copper-wu.flo'

export async function loadDefaultCopperFloTemplate(): Promise<ArrayBuffer> {
...
export async function saveCopperExportBundle(
  folderName: string,
  files: CopperExportBundleFile[]
): Promise<CopperExportBundleResult> {
