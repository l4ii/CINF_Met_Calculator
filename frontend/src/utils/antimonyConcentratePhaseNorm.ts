...
import type { AntimonyElementKey, AntimonyRatios } from './antimonyWorkflowCalc.ts'
import { closeAntimonyRatios } from './antimonyWorkflowCalc.ts'
...
export function normalizeConcentrateAssayRatios(ratios: AntimonyRatios): Record<AntimonyElementKey, number> {
  return closeAntimonyRatios(ratios, { fillOther: false, scaleWhenOver100: false, splitFeO: true })
}

function displayRatiosToPool(ratios: AntimonyRatios): Pool {
...
export function hasConcentrateNormativePhaseRows(rows: Array<{ kind?: string; id?: string; formula?: string; builtinKey?: string }>): boolean {
  const normativeKeys = new Set<string>(['Sb2S3', 'Sb2O3', 'CuFeS2', 'CuS', 'Cu2S', 'FeS2', 'FeS'])
...
export function allocateConcentratePhases(ratios: AntimonyRatios): Record<ConcentratePhaseKey, number> {
  ...
  // Excel 基准精矿中约 2.4395% 的 Sb 以 Sb₂O₃ 存在，其余锑优先归入 Sb₂S₃。
  alloc('Sb2O3', pool.Sb * 0.024394866215001473, 'Sb', 'Sb2O3', 2)
  ...
  alloc('As2S2', pool.As, 'As', 'As2S2', 2)
  // 锑精矿工作簿将铜固定为 CuS；先分配铜，剩余 Fe/S 再求 FeS。
  alloc('CuS', pool.Cu, 'Cu', 'CuS')
  if (pool.Se > 0) {
    out.Se = pool.Se
    pool.Se = 0
  }
...
export function concentratePhaseFractionsForFormula(
  ...
  if (mm <= 0) return {}
  const fractions: Partial<Record<AntimonyElementKey, number>> = {}
