...
export type PhaseSolverResult = {
  ...
  residual: Partial<Record<AntimonyElementKey, number>>
  message?: string
  elementCount: number
  phaseCount: number
}
...
const EXCLUDED_POOL_ELEMENTS = new Set<AntimonyElementKey>([
  'O(氧)',
  'C (碳)',
  'N(氮)',
  'Other(其他)',
...
function collectParticipatingElements(
  ...
  const phaseElements = new Set<AntimonyElementKey>()
  for (const phase of phases) {
    for (const [element, fraction] of Object.entries(phase.fractions) as [AntimonyElementKey, number][]) {
      if (!fraction || fraction <= 0) continue
      if (EXCLUDED_POOL_ELEMENTS.has(element)) continue
      phaseElements.add(element)
    }
  }
  const elements: AntimonyElementKey[] = []
...
function computeResidual(
  ...
  const residual: Partial<Record<AntimonyElementKey, number>> = {}
...
export function solvePhaseDistribution(
  ...
  const residual =
    phaseCount < elementCount ? computeResidual(matrix, vector, solution, elements) : ({} as Partial<Record<AntimonyElementKey, number>>)
