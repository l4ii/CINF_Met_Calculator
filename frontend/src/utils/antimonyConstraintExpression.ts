...
import { productNamesForSolver, type OxySideBlowProductKey } from './antimonyConstraintConfig.ts'
...
export interface ConstraintSymbolTable {
  ...
  /** Input.混合锑精矿 → mass t/h */
  inputMass: Record<string, number>
  /** Input.混合锑精矿.Cu → element mass in feed */
  inputElementMass: Record<string, Partial<Record<AntimonyElementKey, number>>>
  /** Input.混合锑精矿.Cu2S → phase mass in feed (optional) */
  ...
  outputElementMass: Record<string, Partial<Record<AntimonyElementKey, number>>>
}

const ELEMENT_SYMBOL_MAP: Record<string, AntimonyElementKey> = {
  Cu: 'Cu(铜)',
  S: 'S (硫)',
  Fe: 'Fe(铁)',
  SiO2: 'SiO₂(二氧化硅)',
...
function elementMassInPhase(phaseKey: string, token: string): number {
  return resolveElementMass(ANTIMONY_BUILTIN_PHASE_FRACTIONS[phaseKey] as Partial<Record<AntimonyElementKey, number>>, token)
}
...
export function buildConstraintSymbolTable(params: {
  blendMass: number
  blendElementMass: Partial<Record<AntimonyElementKey, number>>
  fuelMass: number
  fuelElementMass: Partial<Record<AntimonyElementKey, number>>
  concentrateMass: number
  inputPhaseMass?: Record<string, Record<string, number>>
  variables?: Record<string, number>
  gasMass?: Record<string, number>
  gasElementMass?: Record<string, Partial<Record<AntimonyElementKey, number>>>
  productNames: Record<OxySideBlowProductKey, string>
  products: Record<
    OxySideBlowProductKey,
    { mass: number; phases: Record<string, number>; elementMass: Partial<Record<AntimonyElementKey, number>> }
  >
