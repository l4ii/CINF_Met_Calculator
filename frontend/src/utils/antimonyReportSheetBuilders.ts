import type { AntimonyBatchExportRow, AntimonyBatchWorkbookSheet } from './antimonyBatchExport.ts'

export interface AntimonyReportElement {
  key: string
  label: string
}

export interface AntimonyReportMaterial {
  header: string
  name: string
  dryWeightTh: number
...
export interface AntimonyReportBlend {
  dryWeightTh: number
  waterWeightTh: number
  composition: Record<string, number | null | undefined>
  compositionTotal: number
...
export interface AntimonyReportProduct {
  name: string
  massTh: number
  composition: Record<string, number | null | undefined>
}

export interface AntimonyReportPhase {
  key: string
  label: string
}

export interface AntimonyReportInputPhaseMaterial {
  header: string
  name: string
  dryWeightTh: number
  waterWeightTh: number
...
const ANNUAL_OPERATING_HOURS = 24 * 330
...
export function buildInputMaterialElementSheet({
  ...
  materials: AntimonyReportMaterial[]
  blend: AntimonyReportBlend
  elements: AntimonyReportElement[]
  ...
  const flowValues = (selector: (material: AntimonyReportMaterial) => number, blendValue: number) => [
    ...materials.map((material) => format(selector(material))),
    format(blendValue * ANNUAL_OPERATING_HOURS),
    '',
  ]
  const rows: AntimonyBatchExportRow[] = [
  ...
  return {
    ...
    columns,
    rows,
    unitNote: '流量 t/h；年量 t/a；元素组成 w%（干基）',
    rowHeaderLabel: '项目',
...
export function buildInputMaterialPhaseSheet({
  ...
  materials: AntimonyReportInputPhaseMaterial[]
  phases: AntimonyReportPhase[]
  format: NumberFormatter
}): AntimonyBatchWorkbookSheet {
  ...
  const materialValues = (selector: (material: AntimonyReportInputPhaseMaterial) => number) =>
...
function balanceElements(elements: AntimonyReportElement[]) {
...
export function buildElementBalanceSheet({
  ...
  inputs: AntimonyReportMaterial[]
  outputs: AntimonyReportProduct[]
  elements: AntimonyReportElement[]
  format: NumberFormatter
}): AntimonyBatchWorkbookSheet | null {
  const activeInputs = inputs.filter((material) => material.name.trim() && material.dryWeightTh > 0)
  const activeOutputs = outputs.filter((product) => product.name.trim() && product.massTh > 0)
  ...
  const buildValues = (
    ...
    name,
    format(hourlyMass * ANNUAL_OPERATING_HOURS),
    format(hourlyMass),
  ...
  const inputRows: AntimonyBatchExportRow[] = activeInputs.map((material, index) => ({
    label: String(index + 1),
    values: buildValues(material.name, material.dryWeightTh, material.composition),
  }))
  const outputRows: AntimonyBatchExportRow[] = activeOutputs.map((product, index) => ({
    label: String(index + 1),
  ...
  return {
    ...
    rows: [
      ...
      ...inputRows,
      totalRow(activeInputs, (material) => material.dryWeightTh),
      { label: '产出', values: [], role: 'section' },
      ...outputRows,
      totalRow(activeOutputs, (product) => product.massTh),
    ],
    unitNote: '物料量 t/a、t/h；元素组成 w%；元素量 t/h',
    rowHeaderLabel: '序号',
