...
function poolKeysForConstraintKey(constraintKey: ConstraintElementKey): AntimonyElementKey[] {
  if (CONSTRAINT_PLACEHOLDER_ELEMENTS.has(constraintKey)) return []
  const binding = resolveConstraintElementBinding(constraintKey)
  const keys = new Set<AntimonyElementKey>([binding.poolKey])
  if ((ANTIMONY_ELEMENT_DISPLAY_ORDER as readonly string[]).includes(constraintKey)) {
    keys.add(constraintKey as AntimonyElementKey)
  }
  return [...keys]
}
...
function compositionPercent(product: OxyProductResult, poolKeys: AntimonyElementKey[]): number {
  if (product.mass <= 0) return 0
  const sum = poolKeys.reduce((acc, key) => acc + (product.composition[key] ?? 0), 0)
  return sum
}

function elementMassTh(product: OxyProductResult, poolKeys: AntimonyElementKey[]): number {
