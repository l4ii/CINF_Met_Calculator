import type { AntimonyElementKey, AntimonyRatios, AntimonyMaterialColumn, PhaseAssistRowSpec } from './antimonyWorkflowCalc.ts'
import {
...
} from './antimonyConcentratePhaseNorm.ts'
import {
  buildBlendPhaseColumn,
  normalizePhasePercents,
  type PhasePercentMap,
} from './antimonyPhaseTableCalc.ts'
import type { MaterialPhaseAssistRow } from './antimonyPhaseAssist.ts'
import { materialPhaseRowDisplayLabel } from './antimonyPhaseAssist.ts'
import { validateMaterialForPhaseCalc } from './antimonyMaterialValidation.ts'
import { getBuiltinPhaseFractions } from './antimonyPhaseTableCalc.ts'
import {
  ANTIMONY_PHASE_TABLE_COMPOUND_KEYS,
} from './antimonyElementDisplay.ts'

export type PhaseUnknownValues = Pick<Record<AntimonyElementKey, number>, 'O(氧)' | 'C (碳)' | 'Other(其他)'>
...
/** 物相区表头元素列（化合物口径） */
export const ANTIMONY_PHASE_TABLE_ELEMENT_KEYS = [...ANTIMONY_PHASE_TABLE_COMPOUND_KEYS] as const
...
/** 混合锑精矿约束输入物相质量流：各原料物相 w% 按投料量直接折算为 t/h */
...
export function traceAssayNotInPhaseRows(
  ratios: AntimonyRatios,
  elementTotalsInPhases: Partial<Record<AntimonyElementKey, number>>
): number {
...
export function validateRawMaterialPhaseInputs(params: {
  ...
  if (!params.blendPhaseMass || Object.keys(params.blendPhaseMass).length === 0) {
    return { ok: false, message: '混合锑精矿物相质量未生成，请完成全部原料物相回填后重试。' }
  }
  return { ok: true }
}
