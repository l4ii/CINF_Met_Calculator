...
import { loadOxyConvertingConstraints } from './copperConstraintConfig.ts'
import {
...
} from './copperProcessStageState.ts'
...
export const CONVERTING_WHITE_MATTE_NAME = '白铜锍'
...
function cloneRatios(ratios: CopperRatios): Record<CopperElementKey, number> {
...
function createMaterialColumn(params: {
  ...
  return {
    ...
    waterWeight: 0,
    moisture: 0,
    ratios: cloneRatios(params.ratios ?? emptyCopperRatios()),
...
export function createConvertingDefaultScrapMaterials(): CopperMaterialColumn[] {
  return [
...
export function createConvertingOxideSlagMaterial(weight = 0): CopperMaterialColumn {
...
export function createConvertingLimeSolventColumn(weight = 0): CopperMaterialColumn {
  const lime = DEFAULT_COPPER_SOLVENTS.find((item) => item.id === 'lime') ?? DEFAULT_COPPER_SOLVENTS[2]!
  return {
...
export function createConvertingWhiteMattePlaceholder(): CopperMaterialColumn {
  return createMaterialColumn({
...
export function createConvertingDefaultRawMaterials(): CopperMaterialColumn[] {
  return [
...
/** 吹炼原料库：白铜锍 + 残极 + 氧化渣 + 石灰石（不含熔炼精矿/石英石） */
export function createConvertingMaterialLibrary(): CopperLibraryMaterial[] {
...
export function createRefiningMaterialLibrary(): CopperLibraryMaterial[] {
  return []
}

export function defaultMaterialLibraryForStage(
  stageId: 'cu_smelting' | 'cu_converting' | 'cu_refining'
): CopperLibraryMaterial[] {
  if (stageId === 'cu_converting') return createConvertingMaterialLibrary()
  if (stageId === 'cu_refining') return createRefiningMaterialLibrary()
  return createSmeltingMaterialLibrary()
}

/** 用吹炼投入列中的白铜锍覆盖/写入原料库条目 */
export function upsertWhiteMatteInLibrary(
  library: CopperLibraryMaterial[],
  matte: CopperMaterialColumn
): CopperLibraryMaterial[] {
  const entry: CopperLibraryMaterial = {
    id: CONVERTING_WHITE_MATTE_ID,
    name: CONVERTING_WHITE_MATTE_NAME,
    category: 'product',
...
function createScrapPhaseSeed(material: CopperMaterialColumn, phasePct?: Record<string, number>): {
  rows: MaterialPhaseAssistRow[]
...
function createOxideSlagPhaseSeed(material: CopperMaterialColumn, phasePct?: Record<string, number>): {
  rows: MaterialPhaseAssistRow[]
  result: PhaseMaterialCalcResult
  ratios: Record<CopperElementKey, number>
} {
...
function createLimePhaseSeed(material: CopperMaterialColumn, phasePct?: Record<string, number>): {
  rows: MaterialPhaseAssistRow[]
  result: PhaseMaterialCalcResult
  ratios: Record<CopperElementKey, number>
} {
...
/** 白铜锍已由熔炼带回且完成物相时，吹炼 O/C 批量计算应跳过 */
...
function matteCompositionToRatios(matte: OxyProductResult): Record<CopperElementKey, number> {
...
function buildMattePhaseSeed(matte: OxyProductResult): {
  ...
  return {
    ...
    result: {
      ...
      message: hasPhases ? undefined : '熔炼白铜锍尚未提供可用物相',
    },
  }
}
...
/** 保证吹炼投入结构：白铜锍 + 残极一/二 + 氧化渣 + 石灰石熔剂；保留已有残极用户改动 */
export function ensureConvertingFeedStructure(state: CopperProcessStageState): CopperProcessStageState {
...
function materialFingerprint(material: CopperMaterialColumn | null | undefined): string {
  if (!material) return ''
  return `${material.weight.toFixed(8)}#${COPPER_ELEMENT_KEYS.map((key) => `${key}:${(material.ratios[key] ?? 0).toFixed(8)}`).join('|')}`
}

function clearConvertingDownstreamResults(state: CopperProcessStageState): void {
...
/**
 * 用熔炼最新白铜锍覆盖吹炼投入中的白铜锍（质量/元素/物相）；保留残极等其余用户改动。
 * 每次切入吹炼都应调用。
 * 熔炼解为 acceptable，或已回填产出且白铜锍质量>0 时均可同步（避免 relaxed 回填后吹炼侧清零）。
 */
export function syncWhiteMatteFromSmelting(
  convertingState: CopperProcessStageState,
  smeltingState: CopperProcessStageState | null | undefined
  ...
  const syncedMatte: CopperMaterialColumn = {
    id: CONVERTING_WHITE_MATTE_ID,
    name: CONVERTING_WHITE_MATTE_NAME,
    kind: 'raw',
    weight: matte.mass,
...
/** 吹炼产出前检查：白铜锍投料量异常偏低或未同步时给出提示（不阻断） */
export function convertingWhiteMatteFeedWarning(
  convertingState: Pick<CopperProcessStageState, 'rawMaterials'>,
  smeltingState?: CopperProcessStageState | null
): string | null {
  ...
  if (weight <= 1e-9) {
    return '吹炼白铜锍投料量为 0：请先完成熔炼产出并同步白铜锍，再计算吹炼产出（粗铜质量会随入炉铜量偏低）。'
  }
  if (smeltingMatteMass > 1e-6 && weight < smeltingMatteMass * 0.85) {
    return `吹炼白铜锍投料 ${weight.toFixed(2)} t/h 明显低于熔炼白铜锍 ${smeltingMatteMass.toFixed(2)} t/h，粗铜产出可能系统性偏低；请确认已同步熔炼结果。`
  }
  return null
}

export function createBlankConvertingProcessStageState(): CopperProcessStageState {
...
export function createProcessStageStateForId(
  ...
  if (stageId === 'cu_converting') return createBlankConvertingProcessStageState()
  if (stageId === 'cu_refining') {
    return {
      ...createBlankProcessStageState(),
 ...
 * 残极按物相反推元素表并写入物相结果；白铜锍仅更新库内化验占位，质量仍以熔炼同步为准。
 */
export function applyMetcalConvertingFeedsToState(
  ...
  const upsertLibraryEntry = (
    id: string,
    name: string,
    category: CopperLibraryMaterial['category'],
    ratios: Record<CopperElementKey, number>,
    unitPrice = 0
  ) => {
    const entry: CopperLibraryMaterial = { id, name, category, ratios: cloneRatios(ratios), unitPrice }
