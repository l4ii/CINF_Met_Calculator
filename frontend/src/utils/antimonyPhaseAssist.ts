...
import { CONCENTRATE_DEFAULT_PHASE_FORMULAS, concentratePhaseFractionsForFormula } from './antimonyConcentratePhaseNorm.ts'
import {
  ANTIMONY_PHASE_OXYGEN_FACTORS,
  ANTIMONY_PHASE_SULFUR_FACTORS,
...
} from './antimonyPhaseTableCalc.ts'
import {
...
import { BATCH_PHASE_ASSIST_MIN_DISPLAY_COLUMNS } from './antimonyBatchTableLayout.ts'
import type { PhasePivotRow } from './antimonyPhaseBatchCalc.ts'
...
export const DEFAULT_BUILTIN_PHASE_ORDER: AntimonyPhaseAssignmentKey[] = [
  ...ANTIMONY_BUILTIN_PHASE_DISPLAY_ORDER.filter(
    (key) => !(['Cu2O', 'FeO', 'Fe2O3', 'Fe3O4'] as AntimonyPhaseAssignmentKey[]).includes(key)
  ),
]
...
const ELEMENT_COMPONENT_PHASE_FRACTIONS: Partial<Record<string, Partial<Record<AntimonyElementKey, number>>>> = {
  H: { 'H(氢)': 1 },
  N: { 'N(氮)': 1 },
  O: { 'O(氧)': 1 },
}

const FORMULA_TO_ASSAY_RATIO_KEY: Partial<Record<string, AntimonyElementKey>> = {
  C: 'C (碳)',
  H: 'H(氢)',
  S: 'S (硫)',
  N: 'N(氮)',
...
export function createMaterialPhaseRowsFromFormulas(formulas: string[]): MaterialPhaseAssistRow[] {
  const rows = formulas.flatMap((formula): MaterialPhaseAssistRow[] => {
    ...
    const builtinKey = ANTIMONY_BUILTIN_PHASE_DISPLAY_ORDER.find(
    ...
    const stoichKey = [trimmed, normalized].find((key) => ANTIMONY_BUILTIN_PHASE_FRACTIONS[key])
    if (stoichKey) {
      return [
        {
          id: `custom:${stoichKey}`,
          ...
          fractions: { ...(ANTIMONY_BUILTIN_PHASE_FRACTIONS[stoichKey] as Partial<Record<AntimonyElementKey, number>>) },
        },
      ]
    }
...
export function createDefaultMaterialPhaseRowsForMaterial(
  material?: Pick<AntimonyMaterialColumn, 'id' | 'name' | 'kind'>
  ...
  if (material?.kind === 'raw' && (id === 'raw-1' || name.includes('精矿'))) {
    return createConcentrateMaterialPhaseRows()
  }
  if (
    id === 'solvent-lime' ||
    id === 'lime' ||
    ...
    return createMaterialPhaseRowsFromFormulas(['CaO', 'Other'])
  }
  if (id === 'solvent-iron-ore' || id === 'iron-ore' || name === '铁矿石') {
    return createMaterialPhaseRowsFromFormulas(['Fe2O3', 'SiO2', 'Al2O3', 'Other'])
  }
...
export function phaseRowSulfurContribution(
  ...
  if (row.kind === 'builtin' && row.builtinKey) {
    return effectivePercent * (ANTIMONY_PHASE_SULFUR_FACTORS[row.builtinKey] ?? 0)
  }
...
export function phaseRowOxygenContribution(row: MaterialPhaseAssistRow, effectivePercent: number): number {
  ...
  if (row.kind === 'other') return 0
  if (row.kind === 'builtin' && row.builtinKey) {
    return effectivePercent * (ANTIMONY_PHASE_OXYGEN_FACTORS[row.builtinKey] ?? 0)
  }
...
function componentPercentForFormula(formula: string, ratios: Record<AntimonyElementKey, number>): number {
  // FeO 已由 normalizeAntimonyRatios(splitFeO) 折入 Fe(铁)/O(氧)，元素键不含 FeO
  if (formula === 'Fe') {
