...
} from './antimonyConstraintElementBridge.ts'
import {
...
import type { ConstraintSymbolTable } from './antimonyConstraintExpression.ts'
import {
  autoFillOxyProductConstraintConfig,
  isBlankConstraintRuleValue,
  resolveConstraintRuleValue,
} from './antimonyConstraintValidation.ts'
import {
...
import {
  ...
  SLAG_FE_SIO2_EXPR,
} from './antimonyProcessParameters.ts'
...
const FLUE_GAS_O2_WT_FRACTION_EXPR = 'Output.烟气.O2 / Output.烟气'
...
export const FUEL_WET_BASIS_WATER_TARGET = 0
...
function resolveElementDistributionFraction(
  config: OxySideBlowConstraintConfig,
  element: ConstraintElementKey,
  product: OxySideBlowProductKey,
  fallback: number
): number {
  const rule = config.elementDistributions
    .find((entry) => entry.element === element)
    ?.rules.find((entry) => entry.product === product && entry.type === 'D%')
  if (!rule) return fallback
  try {
    return Math.min(1, Math.max(0, resolveConfigNumber(rule.value, config.variables, `${element} 分配率`) / 100))
  } catch {
    return fallback
  }
}
...
function resolveFlueGasResidualOxygenTarget(config: OxySideBlowConstraintConfig): number {
  ...
  const directWeightFraction = config.customConstraints.find(
    (entry) => normalizedConstraintExpr(entry.expr) === normalizedConstraintExpr(FLUE_GAS_O2_WT_FRACTION_EXPR)
  )
  if (typeof directWeightFraction?.target === 'number' && Number.isFinite(directWeightFraction.target)) {
    return Math.min(0.95, Math.max(0, directWeightFraction.target))
  }
...
export function derivedFuelDryMass(baseInput: OxyConstraintBaseInput, config: OxySideBlowConstraintConfig): number {
  ...
  return resolveFuelConcentrateBasisMass(baseInput) * resolveFuelConcentrateRatioTarget(config)
}

/** Excel 的煤/精矿比以湿精矿为分母；无原料明细时兼容旧调用的 concentrateMass。 */
function resolveFuelConcentrateBasisMass(baseInput: OxyConstraintBaseInput): number {
  if (baseInput.rawMaterialColumns?.length) {
    const { concentrates } = partitionRawMixMaterials(baseInput.rawMaterialColumns)
    const wetMass = concentrates.reduce((sum, material) => sum + materialWetWeight(material), 0)
    if (wetMass > 0) return wetMass
  }
  return Math.max(0, baseInput.concentrateMass)
}
...
export interface OxyConstraintBaseInput {
  ...
  rawMaterialColumns?: AntimonyMaterialColumn[]
  concentrateMass: number
  /**
   * true：产出求解按 fuelColumn.weight 使用外部传入煤量。
   * 用于热平衡闭合：煤量由热收入/支出差反算，再联动产出约束复核。
   * false/undefined：兼容旧流程，按 customConstraints 中“煤/混合锑精矿”目标派生煤量。
   */
...
function closeRatiosForBalance(ratios: Partial<Record<AntimonyElementKey, number>>): Record<AntimonyElementKey, number> {
...
function solverWaterWeight(column: AntimonyMaterialColumn): number {
...
function columnBalanceElementMass(column: AntimonyMaterialColumn): Partial<Record<AntimonyElementKey, number>> {
  const dryWeight = Math.max(0, column.weight)
  const ratios = closeRatiosForBalance(column.ratios)
  const displayMass = Object.fromEntries(
    ANTIMONY_ELEMENT_KEYS.map((el) => [el, (dryWeight * (ratios[el] ?? 0)) / 100])
...
function calculateBalanceWeightedComposition(materials: AntimonyMaterialColumn[]): WeightedComposition {
...
function outputBoundaryElementMass(column: AntimonyMaterialColumn): Partial<Record<AntimonyElementKey, number>> {
  return columnBalanceElementMass(column)
}

function calculateOutputBoundaryWeightedComposition(materials: AntimonyMaterialColumn[]): WeightedComposition {
  const totalWeight = materials.reduce(
    (sum, material) => sum + Math.max(0, material.weight) + solverWaterWeight(material),
    0
  )
...
function calculateSolverDisplayComposition(materials: AntimonyMaterialColumn[]): WeightedComposition {
  const totalWeight = materials.reduce(
    (sum, material) => sum + Math.max(0, material.weight) + solverWaterWeight(material),
    0
  )
  const elementWeights = emptyAntimonyRatios()
  if (totalWeight <= 0) return { totalWeight: 0, elementWeights, ratios: emptyAntimonyRatios() }
  const waterRatios = waterElementRatios()
  for (const material of materials) {
...
function calculateInputWaterMass(materials: AntimonyMaterialColumn[]): number {
...
function balanceFeedFromDisplayFeed(feed: WeightedComposition): WeightedComposition {
  ...
  } as Record<AntimonyElementKey, number>
  const ratios = emptyAntimonyRatios()
  for (const element of ANTIMONY_ELEMENT_KEYS) {
...
function resolveRawBalanceFeed(baseInput: OxyConstraintBaseInput, rawDisplayFeed: WeightedComposition): WeightedComposition {
  if (baseInput.rawMaterialColumns?.length) {
    return calculateBalanceWeightedComposition(
      baseInput.rawMaterialColumns.map((material) => ({
        ...material,
        waterWeight: 0,
        moisture: 0,
      }))
...
function phaseFormulaFractions(phaseKey: string): Partial<Record<AntimonyElementKey, number>> {
  if (phaseKey === 'Other') return { 'Other(其他)': 1 }
  const normalizedKey = normalizeMetcalPhaseFormula(phaseKey) || phaseKey
  const builtin =
    (ANTIMONY_BUILTIN_PHASE_FRACTIONS[normalizedKey] as Partial<Record<AntimonyElementKey, number>> | undefined) ??
    (ANTIMONY_BUILTIN_PHASE_FRACTIONS[phaseKey] as Partial<Record<AntimonyElementKey, number>> | undefined)
  if (builtin && Object.keys(builtin).length > 0) return builtin
  return phaseFractionsFromFormula(normalizedKey) as Partial<Record<AntimonyElementKey, number>>
}
...
function isAntimonyElementKey(key: string): key is AntimonyElementKey {
...
function constrainedOutputElementSpecs(
  ...
  for (const entry of preparedConfig.elementDistributions) {
    ...
    if (!isAntimonyElementKey(binding.poolKey)) continue
...
function deriveElementMassFromPhases(phases: Record<string, number>): Partial<Record<AntimonyElementKey, number>> {
  ...
  for (const [phaseKey, mass] of Object.entries(phases)) {
    ...
    for (const [el, frac] of Object.entries(fracs) as [AntimonyElementKey, number][]) {
...
function findAirColumn(airColumns: AntimonyMaterialColumn[], name: string) {
  return airColumns.find((col) => col.name === name)
}

function phaseFraction(phaseKey: string, element: AntimonyElementKey): number {
...
function phaseMassForElement(phases: Record<string, number>, phaseKey: string, element: AntimonyElementKey): number {
...
function inputPhaseMass(baseInput: OxyConstraintBaseInput): Record<string, number> {
  return (
    baseInput.inputPhaseMass?.混合锑精矿 ??
    Object.values(baseInput.inputPhaseMass ?? {})[0] ??
    {}
  )
}

function columnElementFraction(column: AntimonyMaterialColumn | undefined, element: AntimonyElementKey): number {
  if (!column) return 0
  const unitColumn = { ...column, weight: 1, waterWeight: 0, moisture: 0 }
  return columnBalanceElementMass(unitColumn)[element] ?? 0
}

function gasPhaseMasses(column: AntimonyMaterialColumn): Record<string, number> {
  const dryWeight = Math.max(0, column.weight)
  const ratios = closeRatiosForBalance(column.ratios)
  return {
...
function gasMoleRatesPerDryTon(column: AntimonyMaterialColumn | undefined): { o2: number; total: number } {
...
function oxygenGasMassForVolumeFraction(
  totalDryMass: number,
  processAir: AntimonyMaterialColumn | undefined,
  oxygen: AntimonyMaterialColumn | undefined,
...
function adjustSecondaryAirSupply(
  ...
  const hasSecondaryAirConstraint = config.customConstraints.some(
    (entry) =>
      normalizedConstraintExpr(entry.expr) === normalizedConstraintExpr(SECONDARY_AIR_OXYGEN_SUPPLY_EXPR)
  )
  if (!hasSecondaryAirConstraint) {
    unpacked.gasMass['二次风'] = 0
    return
  }
...
function adjustSmeltingPrimaryOxygenSupply(
  ...
  const iron = Math.max(0, unpacked.rawBalanceFeed.elementWeights['Fe(铁)'] ?? 0)
  const sulfurToSO2 = sulfur * resolveElementDistributionFraction(
    config,
    'S (硫)',
    'flueGas',
    0.855
  )
  const ironToSlag = iron * resolveElementDistributionFraction(
    config,
    'Fe(铁)',
    'smeltingSlag',
    0.523147528461926
...
function applyInitialSlagPhaseGuess(unpacked: UnpackedUnknowns) {
  ...
  const shares: Record<string, number> = {
    Sb2O3: 0.212248997371663,
    S: 0.0117,
    FeO: 0.280358419749933,
    PbO: 0.00129266061776062,
    As2O3: 0.00937429219343954,
    Bi2O3: 0.000200671019865978,
    ZnO: 0.00224048516365861,
    Cu2O: 0.000225177666572247,
    SiO2: 0.215660322884564,
    CaO: 0.107830161442282,
    Al2O3: 0.0822790754846596,
    Ag: 0.000014,
    Au: 0.00000586378631379195,
    Other: 0.076569872619288,
  }
  for (const phaseKey of Object.keys(phases)) {
    phases[phaseKey] = total * Math.max(0, shares[phaseKey] ?? 0)
  }
}
...
function applyInitialConvertingSlagPhaseGuess(unpacked: UnpackedUnknowns) {
  ...
  if (cu2oCu > 0) {
    const cu2oAntimony = (slagCuShare * total) / (1 + cu2sCu2oRatio * (cu2sCu / cu2oCu))
    phases.Cu2O = Math.max(0, cu2oAntimony / cu2oCu)
...
function applyInitialMattePhaseGuess(unpacked: UnpackedUnknowns) {
  const phases = unpacked.outputPhases.matte
  const total = productPhaseMass(phases)
  if (total <= 0) return
  const shares: Record<string, number> = {
    Sb2S3: 0.0850943737166325,
    FeS: 0.659627721710905,
    As: 0.0069,
    CaO: 0.0619839675648702,
    SiO2: 0.0858791850481717,
    PbS: 0.00434019678687429,
    Cu2S: 0.000839320100200731,
    Ag: 0.00005,
    Other: 0.0952852350723454,
  }
  for (const phaseKey of Object.keys(phases)) {
    phases[phaseKey] = total * Math.max(0, shares[phaseKey] ?? 0)
  }
...
/** 粗铜初值：金属 Cu 为主相，Cu2O/Cu2S 按 O/S 品位，不再套用锑锍 Cu2S-GMC 模型 */
function applyInitialBlisterAntimonyPhaseGuess(unpacked: UnpackedUnknowns, config: OxySideBlowConstraintConfig) {
  const phases = unpacked.outputPhases.matte
  if (phases.Cu === undefined) {
    applyInitialMattePhaseGuess(unpacked)
    return
  }
  ...
  const antimonyFromOxides =
    Math.max(0, phases.Cu2O) * cu2oCu + Math.max(0, phases.Cu2S) * cu2sCu
  phases.Cu = Math.max(0, totalHint * (gmc / 100) - antimonyFromOxides)
...
function applyInitialDustPhaseGuess(unpacked: UnpackedUnknowns) {
  ...
  if (total <= 0) return
  const shares: Record<string, number> = {
    Sb2O3: 0.894255351209323,
    PbO: 0.00420114700772201,
    As2O3: 0.00645026829881736,
    Bi2O3: 0.0000612215081156289,
    CaO: 0.0149712969061876,
    SiO2: 0.0342152181462573,
    FeO: 0.0244432395652407,
    ZnO: 0.000506266811942785,
    Cu2O: 0.000112588833286123,
    S: 0.0167,
    Ag: 0.0000340478340413497,
    Other: 0.00404935387906659,
  }
  for (const phaseKey of Object.keys(phases)) {
    phases[phaseKey] = total * Math.max(0, shares[phaseKey] ?? 0)
  }
...
function applyHardDustPhaseElementClosure(unpacked: UnpackedUnknowns) {
  ...
  const freeElementPhaseHint: Partial<Record<AntimonyElementKey, string>> = {
    'Se(硒)': 'SeO2',
    'Te(碲)': 'Te',
    'Other(其他)': 'Other',
  }
  ...
  if (free) {
    for (const [elementKey, raw] of Object.entries(free) as [AntimonyElementKey, number][]) {
...
function applyInitialOutputPhaseGuess(unpacked: UnpackedUnknowns, config: OxySideBlowConstraintConfig) {
  if (isOxyConvertingConstraintConfig(config)) {
    ...
    applyInitialBlisterAntimonyPhaseGuess(unpacked, config)
  } else {
...
function applyHardSlagPhaseConstraints(unpacked: UnpackedUnknowns, config: OxySideBlowConstraintConfig) {
  const phases = unpacked.outputPhases.smeltingSlag
  if (productPhaseMass(phases) <= 1e-12) return

  const feSiO2Target = Math.max(
    1e-6,
    resolveCustomConstraintTarget(config, SLAG_FE_SIO2_EXPR, 1.010461956)
  )
  const caOSiO2Target = Math.max(
    0,
    resolveCustomConstraintTarget(config, SLAG_CAO_SIO2_EXPR, 0.5)
  )
  ...
  const fixedIron =
    Math.max(0, phases.FeS ?? 0) * feSFe
  phases.FeO = Math.max(0, (feSiO2Target * silicaEquivalent - fixedIron) / feOFe)
  if (phases.CaO !== undefined) {
    phases.CaO = Math.max(0, caOSiO2Target * silicaEquivalent)
  }
}
...
/**
 * 吹炼锑锍中的 Other 是未分析相的质量闭合项，不应被强制完整地留在最终 Other 相。
...
function applyHardOutputPhaseConstraints(unpacked: UnpackedUnknowns, config: OxySideBlowConstraintConfig) {
  ...
  flueGas.O2 = isOxyConvertingConstraintConfig(config)
    ? Math.max(0, gasOxygen * resolveFlueGasResidualOxygenTarget(config))
    : 0
  const n2Fraction = phaseFraction('N2', 'N(氮)')
  if (n2Fraction > 0) {
  ...
  applyHardFlueGasSulfurClosure(unpacked)
  if (!isOxyConvertingConstraintConfig(config)) {
    const oxygenWeightFraction = Math.min(0.95, Math.max(0, resolveFlueGasResidualOxygenTarget(config)))
    const gasMassWithoutOxygen = Object.entries(flueGas).reduce(
      (sum, [phaseKey, mass]) => sum + (phaseKey === 'O2' ? 0 : Math.max(0, mass)),
      0
    )
    flueGas.O2 =
      oxygenWeightFraction > 0
        ? (gasMassWithoutOxygen * oxygenWeightFraction) / (1 - oxygenWeightFraction)
        : 0
  }
...
export function unpackUnknowns(
  ...
  const outputElementMasses = Object.fromEntries(
    OXY_SIDE_BLOW_PRODUCT_KEYS.map((pk) => [pk, {} as Partial<Record<AntimonyElementKey, number>>])
  ) as Record<OxySideBlowProductKey, Partial<Record<AntimonyElementKey, number>>>
...
function mergeElementMasses(
  ...sources: Array<Partial<Record<AntimonyElementKey, number>> | undefined>
): Partial<Record<AntimonyElementKey, number>> {
  const out: Partial<Record<AntimonyElementKey, number>> = {}
  for (const source of sources) {
    if (!source) continue
    for (const [key, value] of Object.entries(source) as [AntimonyElementKey, number][]) {
...
export function buildSymbolTableFromUnknowns(
  ...
  const outputElementMass: Record<string, Partial<Record<AntimonyElementKey, number>>> = {}
  for (const pk of OXY_SIDE_BLOW_PRODUCT_KEYS) {
  ...
  return {
    variables,
    inputMass: {
      混合锑精矿: resolveFuelConcentrateBasisMass(baseInput),
    ...
    inputElementMass: {
      混合锑精矿: unpacked.rawBalanceFeed.elementWeights,
