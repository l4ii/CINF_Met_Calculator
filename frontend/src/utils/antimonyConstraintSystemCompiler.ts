...
import { evaluateConstraintExprString, evaluateOxygenEnrichmentRatio, type ConstraintSymbolTable } from './antimonyConstraintExpression.ts'
import {
...
export function compileOxyConstraintSystem(
  ...
  for (const feedKey of ANTIMONY_ELEMENT_KEYS) {
...
export function evaluateEquationResidual(
  ...
  table: ConstraintSymbolTable,
  config: OxySideBlowConstraintConfig,
  distributionFeedElementWeights: Partial<Record<AntimonyElementKey, number>>,
  balanceFeedElementWeights: Partial<Record<AntimonyElementKey, number>> = distributionFeedElementWeights
): number {
  ...
  switch (equation.kind) {
    case 'D%': {
      ...
      const feedMetal = constraintFeedMetalMass(equation.constraintElement, {
        totalWeight: 0,
        elementWeights: distributionFeedElementWeights as Record<AntimonyElementKey, number>,
        ratios: {} as Record<AntimonyElementKey, number>,
      })
