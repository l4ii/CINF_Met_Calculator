import rawConstraints from '../config/antimonyOxySideBlowConstraints.json' with { type: 'json' }
import rawConvertingConstraints from '../config/antimonyOxyConvertingConstraints.json' with { type: 'json' }
...
function phaseElementKeys(phaseKey: string): ConstraintElementKey[] {
  ...
  const builtin = ANTIMONY_BUILTIN_PHASE_FRACTIONS[normalizedKey] ?? ANTIMONY_BUILTIN_PHASE_FRACTIONS[phaseKey] ?? {}
  const fractions =
...
export function rewriteConstraintExprToConvertingProductNames(expr: string): string {
  ...
  out = out.replace(/OutputE\.锑锍/g, 'OutputE.粗铜')
  out = out.replace(/Output\.锑锍/g, 'Output.粗铜')
...
export function rewriteConstraintExprToSmeltingProductNames(expr: string): string {
  ...
  out = out.replace(/OutputE\.粗铜/g, 'OutputE.锑锍')
  out = out.replace(/Output\.粗铜/g, 'Output.锑锍')
  return out
}
