import type { AntimonyProductKey, AntimonyProductResult } from './antimonyProcessCalc'
import type { AntimonyElementKey } from './antimonyWorkflowCalc'
...
import { solvePhaseDistribution } from './antimonyPhaseSolver.ts'
...
import { sortAntimonyPhaseKeys } from './antimonyDisplayOrder.ts'

function sortProductPhaseRows(rows: string[]) {
  return sortAntimonyPhaseKeys(rows)
}

export const PRODUCT_PHASE_ROWS: Record<AntimonyProductKey, ProductPhaseRowKey[]> = {
  matte: sortProductPhaseRows(['Sb2S3', 'FeS', 'As', 'CaO', 'SiO2', 'PbS', 'Cu2S', 'Ag', 'Other']),
  slag: sortProductPhaseRows([
    'Sb2O3', 'S', 'FeO', 'PbO', 'As2O3', 'Bi2O3', 'ZnO', 'Cu2O',
    'SiO2', 'CaO', 'Al2O3', 'Ag', 'Au', 'Other',
  ]),
  gas: sortProductPhaseRows(['SO2', 'CO2', 'O2', 'N2']),
  dust: sortProductPhaseRows([
    'Sb2O3', 'PbO', 'As2O3', 'Bi2O3', 'CaO', 'SiO2', 'FeO', 'ZnO',
    'Cu2O', 'S', 'Ag', 'Other',
  ]),
  loss: sortProductPhaseRows(['Sb', 'As', 'S', 'Fe', 'Pb', 'Cu', 'Ag', 'Au', 'Other']),
}
...
function builtinPhaseSpecs(keys: string[]) {
  return keys
    .filter((key) => key !== 'Other' && ANTIMONY_BUILTIN_PHASE_FRACTIONS[key])
    .map((key) => ({
      id: key,
...
const POOL_EXCLUDED_FROM_LINEAR_SOLVE = new Set<AntimonyElementKey>([
  'O(氧)',
  'C (碳)',
  'N(氮)',
  'Other(其他)',
...
export function elementPoolForPhaseSolve(
  ...
  for (const [element, weight] of Object.entries(elementWeights) as [AntimonyElementKey, number][]) {
...
function elementPoolFromWeights(elementWeights: Partial<Record<AntimonyElementKey, number>>) {
...
export function calculateProductPhaseComposition(
  productResult: AntimonyProductResult,
  overrides: Partial<Record<AntimonyProductKey, ProductPhasePercentMap>> = {}
): Record<AntimonyProductKey, ProductPhasePercentMap> {
  const out = {} as Record<AntimonyProductKey, ProductPhasePercentMap>
...
const PHASE_TO_ELEMENT_MASS: Record<string, Partial<Record<AntimonyElementKey, (mass: number) => number>>> = {
  Sb2S3: {
    'Sb(锑)': (m) => m * (ANTIMONY_BUILTIN_PHASE_FRACTIONS.Sb2S3['Sb(锑)'] ?? 0),
    'S (硫)': (m) => m * (ANTIMONY_BUILTIN_PHASE_FRACTIONS.Sb2S3['S (硫)'] ?? 0),
  },
  Cu2S: {
    'Cu(铜)': (m) => m * (ANTIMONY_BUILTIN_PHASE_FRACTIONS.Cu2S['Cu(铜)'] ?? 0),
    'S (硫)': (m) => m * (ANTIMONY_BUILTIN_PHASE_FRACTIONS.Cu2S['S (硫)'] ?? 0),
  },
  FeS: {
    'Fe(铁)': (m) => m * (ANTIMONY_BUILTIN_PHASE_FRACTIONS.FeS['Fe(铁)'] ?? 0),
    'S (硫)': (m) => m * (ANTIMONY_BUILTIN_PHASE_FRACTIONS.FeS['S (硫)'] ?? 0),
  },
  Cu2O: {
    'Cu(铜)': (m) => m * (ANTIMONY_BUILTIN_PHASE_FRACTIONS.Cu2O['Cu(铜)'] ?? 0),
    'O(氧)': (m) => m * (ANTIMONY_BUILTIN_PHASE_FRACTIONS.Cu2O['O(氧)'] ?? 0),
  },
  FeO: {
    'Fe(铁)': (m) => m * (ANTIMONY_BUILTIN_PHASE_FRACTIONS.FeO['Fe(铁)'] ?? 0),
    'O(氧)': (m) => m * (ANTIMONY_BUILTIN_PHASE_FRACTIONS.FeO['O(氧)'] ?? 0),
  },
  SiO2: { 'SiO₂(二氧化硅)': (m) => m },
  CaO: { 'CaO(氧化钙)': (m) => m },
  Al2O3: { 'Al₂O₃(三氧化二铝)': (m) => m },
  PbO: {
    'Pb(铅)': (m) => m * (ANTIMONY_BUILTIN_PHASE_FRACTIONS.PbO['Pb(铅)'] ?? 0),
    'O(氧)': (m) => m * (ANTIMONY_BUILTIN_PHASE_FRACTIONS.PbO['O(氧)'] ?? 0),
  },
  As2O3: {
    'As(砷)': (m) => m * (ANTIMONY_BUILTIN_PHASE_FRACTIONS.As2O3['As(砷)'] ?? 0),
    'O(氧)': (m) => m * (ANTIMONY_BUILTIN_PHASE_FRACTIONS.As2O3['O(氧)'] ?? 0),
  },
  Bi2O3: {
    'Bi(铋)': (m) => m * (ANTIMONY_BUILTIN_PHASE_FRACTIONS.Bi2O3['Bi(铋)'] ?? 0),
    'O(氧)': (m) => m * (ANTIMONY_BUILTIN_PHASE_FRACTIONS.Bi2O3['O(氧)'] ?? 0),
  },
  Sb2O3: {
    'Sb(锑)': (m) => m * (ANTIMONY_BUILTIN_PHASE_FRACTIONS.Sb2O3['Sb(锑)'] ?? 0),
    'O(氧)': (m) => m * (ANTIMONY_BUILTIN_PHASE_FRACTIONS.Sb2O3['O(氧)'] ?? 0),
  },
  ZnO: {
    'Zn(锌)': (m) => m * (ANTIMONY_BUILTIN_PHASE_FRACTIONS.ZnO['Zn(锌)'] ?? 0),
    'O(氧)': (m) => m * (ANTIMONY_BUILTIN_PHASE_FRACTIONS.ZnO['O(氧)'] ?? 0),
  },
  SO2: {
  ...
  N2: { 'N(氮)': (m) => m },
  PbS: {
    'Pb(铅)': (m) => m * (ANTIMONY_BUILTIN_PHASE_FRACTIONS.PbS['Pb(铅)'] ?? 0),
    'S (硫)': (m) => m * (ANTIMONY_BUILTIN_PHASE_FRACTIONS.PbS['S (硫)'] ?? 0),
  },
  Sb: { 'Sb(锑)': (m) => m },
  As: { 'As(砷)': (m) => m },
  S: { 'S (硫)': (m) => m },
...
export function deriveProductElementsFromPhases(
  ...
  for (const row of rows) {
    ...
    if (!mapping) continue
    for (const [element, fn] of Object.entries(mapping) as [AntimonyElementKey, (mass: number) => number][]) {
...
export function isProductPhaseColumnValid(phases: ProductPhasePercentMap, productKey: AntimonyProductKey, tolerance = 0.02) {
