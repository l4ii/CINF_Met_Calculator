...
import type { AntimonyChemicalHeatMode, AntimonyHeatBalanceResult } from './antimonyHeatBalance.ts'
import {
...
import type { PhaseBatchResults } from './antimonyPhaseBatchCalc'
import type { CustomPhaseRow } from './antimonyPhaseTableCalc'
import type { MaterialPhaseAssistRow } from './antimonyPhaseAssist'

export type AntimonyProcessStageId = Extract<SheetId, 'sb_smelting' | 'sb_converting' | 'sb_refining'>

export const ANTIMONY_PROCESS_STAGE_IDS: AntimonyProcessStageId[] = [
  'sb_smelting',
  'sb_converting',
  'sb_refining',
]
...
export type PhasePreviewUnknowns = {
  ...
  values: Pick<Record<AntimonyElementKey, number>, 'O(氧)' | 'C (碳)' | 'Other(其他)'>
}

export type ProductDistributionDrafts = Partial<
  Record<AntimonyElementKey, Partial<Record<string, string>>>
>

export type AntimonyProcessStageState = {
  rawMaterials: AntimonyMaterialColumn[]
  rawWeightDrafts: Record<string, string>
  solventColumns: AntimonyMaterialColumn[]
  /** 本工序原料库（熔炼精矿 vs 吹炼锑锍/残极 等彼此独立） */
  materialLibrary: AntimonyLibraryMaterial[]
...
export type AntimonyCaseProcessStages = Partial<Record<AntimonyProcessStageId, AntimonyProcessStageState>>

export type AntimonyCaseSharedPersistedFields = {
...
export function normalizeChemicalHeatMode(value: unknown): AntimonyChemicalHeatMode {
  return value === 'reaction' ? 'reaction' : 'hess'
}

export function isAntimonyProcessStageSheet(sheet: SheetId): sheet is AntimonyProcessStageId {
  return ANTIMONY_PROCESS_STAGE_IDS.includes(sheet as AntimonyProcessStageId)
}

export function processStageIdForSheet(sheet: SheetId): AntimonyProcessStageId | null {
  if (sheet === 'sb_smelting' || sheet === 'sb_smelting_equipment') return 'sb_smelting'
  if (sheet === 'sb_converting' || sheet === 'sb_converting_equipment') return 'sb_converting'
  if (sheet === 'sb_refining' || sheet === 'sb_refining_equipment') return 'sb_refining'
  return null
}
...
/** 配料总表页签在流程中的先后（越大越靠后） */
function batchTableViewProgressRank(view: BatchTableView, stageId: AntimonyProcessStageId): number {
...
function normalizeStageLandingBatchTableView(
  view: BatchTableView,
  stageId: AntimonyProcessStageId
): BatchTableView {
  if (stageId !== 'sb_converting') return view
...
function isConvertingFeedReady(state: AntimonyProcessStageState): boolean {
  return (
...
function isSmeltingPhaseReady(state: AntimonyProcessStageState): boolean {
...
export function createBlankProcessStageState(): AntimonyProcessStageState {
  const defaultRawMaterials = createDefaultAntimonyMaterials()
  ...
  return {
    ...
    targetFeSiO2: String(DEFAULT_ANTIMONY_PROCESS_PARAMETERS.feSiO2),
    targetCaOSiO2: '0.5',
    processParameters: { ...DEFAULT_ANTIMONY_PROCESS_PARAMETERS },
    ...
    fuelLhv: String(DEFAULT_ANTIMONY_FUEL.lowerHeatingValueMJkg),
    fuelEfficiency: String(DEFAULT_ANTIMONY_FUEL.combustionEfficiency),
    oxygenAirO2Pct: DEFAULT_OXYGEN_AIR_O2_TEXT,
    oxygenAirN2Pct: DEFAULT_OXYGEN_AIR_N2_TEXT,
    oxygenSupplyCoefficient: '1.15',
    feedTemperature: '25',
    matteTemperature: '1150',
    slagTemperature: '1150',
    gasTemperature: '1100',
    dustTemperature: '1100',
    lossTemperature: '1150',
    coolingWaterInletTemperature: '25',
    coolingWaterOutletTemperature: '35',
    ...
    productDistributionDrafts: productModelToDrafts(DEFAULT_ANTIMONY_PRODUCT_MODEL),
...
function productModelToDrafts(model: Partial<AntimonyProductModel> = DEFAULT_ANTIMONY_PRODUCT_MODEL): ProductDistributionDrafts {
  ...
  for (const [element, distribution] of Object.entries(model)) {
    ...
    drafts[element as AntimonyElementKey] = Object.fromEntries(
...
export function cloneProcessStageState(state: AntimonyProcessStageState): AntimonyProcessStageState {
  return JSON.parse(JSON.stringify(state)) as AntimonyProcessStageState
}

export function isProcessStageProductReady(state: AntimonyProcessStageState | null | undefined): boolean {
  return Boolean(
...
export function isProcessStageHeatBalanceReady(state: AntimonyProcessStageState | null | undefined): boolean {
  return Boolean(state?.heatBalanced && state?.heatBalanceFilledBack && state?.calculatedHeatBalance)
}

/** 工序产出 + 热平衡均完成，才可进入该工序设备选型 */
export function isProcessStageComplete(state: AntimonyProcessStageState | null | undefined): boolean {
  return isProcessStageProductReady(state) && isProcessStageHeatBalanceReady(state)
}

export function hasProcessStageGeneratedData(state: AntimonyProcessStageState | null | undefined): boolean {
...
export type AntimonyEquipmentStageKey = 'smelting' | 'converting' | 'refining'

export function isEquipmentBomGenerated(
  bomGenerated: Partial<Record<AntimonyEquipmentStageKey, boolean>> | null | undefined,
  stage: AntimonyEquipmentStageKey
): boolean {
 ...
 */
export function antimonyStageUnlockBlockReason(params: {
  targetSheet: SheetId
  getProcessState: (stageId: AntimonyProcessStageId) => AntimonyProcessStageState | null | undefined
  equipmentBomGenerated?: Partial<Record<AntimonyEquipmentStageKey, boolean>> | null
}): string | null {
  const target = params.targetSheet
  if (target === 'raw_material' || target === 'sb_smelting') return null

  const smelting = params.getProcessState('sb_smelting')
  const converting = params.getProcessState('sb_converting')
  const bom = params.equipmentBomGenerated

  if (target === 'sb_smelting_equipment') {
  ...
  if (target === 'sb_converting_equipment') {
  ...
  if (target === 'sb_refining' || target === 'sb_refining_equipment') {
  ...
  if (target === 'sb_summary' || target === 'sb_equipment') {
    if (!isEquipmentBomGenerated(bom, 'smelting')) {
      return '请先完成熔炼设备选型并生成 BOM 清单，再进入案例汇总。'
    }
    return null
  }
...
export function extractLegacyFlatProcessStageState(record: Record<string, unknown>): AntimonyProcessStageState {
  const blank = createBlankProcessStageState()
  const rawMaterials = Array.isArray(record.rawMaterials) ? (record.rawMaterials as AntimonyMaterialColumn[]) : blank.rawMaterials
  const airColumns = Array.isArray(record.airColumns)
    ? (record.airColumns as AntimonyMaterialColumn[])
    : blank.airColumns
  return {
    ...blank,
    rawMaterials,
    ...
    solventColumns: Array.isArray(record.solventColumns)
      ? (record.solventColumns as AntimonyMaterialColumn[])
      : blank.solventColumns,
    materialLibrary: Array.isArray(record.materialLibrary)
      ? (record.materialLibrary as AntimonyLibraryMaterial[])
      : blank.materialLibrary,
    fuelColumn: (record.fuelColumn as AntimonyFuelMaterial | undefined) ?? blank.fuelColumn,
    airColumns,
    ...
    processParameters: (record.processParameters as AntimonyProcessParameters | undefined) ?? blank.processParameters,
    ...
    solventSolution: (record.solventSolution as AntimonySolventSolution | null | undefined) ?? null,
    ...
    calculatedHeatBalance: (record.calculatedHeatBalance as AntimonyHeatBalanceResult | null | undefined) ?? null,
...
export function resolveCaseProcessStages(
  record: { processStages?: AntimonyCaseProcessStages; rawMaterials?: unknown } | null | undefined
): AntimonyCaseProcessStages {
  const stored = record?.processStages
  if (stored && Object.keys(stored).length > 0) {
    const resolved: AntimonyCaseProcessStages = {}
      ...
      if (stored[stageId]) {
        ...
        if (!Array.isArray(cloned.materialLibrary) || cloned.materialLibrary.length === 0) {
          cloned.materialLibrary =
            stageId === 'sb_smelting' ? createSmeltingMaterialLibrary() : []
        }
  ...
  if (record && Array.isArray(record.rawMaterials)) {
    return { sb_smelting: extractLegacyFlatProcessStageState(record as Record<string, unknown>) }
  }
  return { sb_smelting: createBlankProcessStageState() }
}
...
export function buildPersistedCaseContent(record: {
  ...
  smeltingJacketCountOneSide?: string
  smeltingAnnualFeedOverridden?: boolean
}): AntimonyCaseSharedPersistedFields {
...
export function serializePersistedCaseContent(content: AntimonyCaseSharedPersistedFields): string {
...
export function normalizePersistedContentForDirtyCheck(
  content: AntimonyCaseSharedPersistedFields
): AntimonyCaseSharedPersistedFields {
  const processStages: AntimonyCaseProcessStages = {}
  for (const stageId of ANTIMONY_PROCESS_STAGE_IDS) {
...
export function isPersistedCaseContentDirty(
  current: AntimonyCaseSharedPersistedFields,
  saved: AntimonyCaseSharedPersistedFields
): boolean {
  return (
