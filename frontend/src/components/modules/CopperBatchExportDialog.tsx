...
export type CopperBatchExportSelection = {
  excel: boolean
  flo: boolean
  processText: boolean
  sheetKeys: CopperBatchExportSheetKey[]
...
const DEFAULT_GROUP_SELECTION: Record<CopperBatchExportGroupKey, boolean> = {
  input: true,
  output: true,
  heatBalance: true,
}
...
export function CopperBatchExportDialog({
  ...
  onCancel: () => void
  onConfirm: (selection: CopperBatchExportSelection) => void
}) {
  const [activeTab, setActiveTab] = useState<ExportTab>('excel')
  const [selectedGroups, setSelectedGroups] = useState<Record<CopperBatchExportGroupKey, boolean>>({
  ...
  const toggleGroup = (key: CopperBatchExportGroupKey) => {
