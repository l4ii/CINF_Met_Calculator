...
import { BATCH_TABLE_MASS_COL_WIDTH, BATCH_TABLE_PCT_COL_WIDTH } from '../../../../utils/antimonyBatchTableLayout'
import {
...
} from '../../../../utils/antimonyHeatBalance'
import {
  calculateAntimonyHeatAuxiliaryWithTrace,
  formatAuxiliaryParam,
} from '../../../../utils/antimonyHeatAuxiliaryParams.ts'
import {
  HEAT_AUXILIARY_EXPLAIN_ITEMS,
  type HeatAuxiliaryParamKey,
} from '../../../../utils/antimonyHeatAuxiliaryExplain.ts'
...
function outputComponentHeatPriority(section: string) {
  ...
  if (section.includes('锑锍') || section.includes('粗铜') || section.includes('铜锍')) return 1
  if (
...
function ReactionEquationTable({ darkMode, result }: { darkMode: boolean; result: AntimonyHeatBalanceResult }) {
...
function ReactionTable({ darkMode, result }: { darkMode: boolean; result: AntimonyHeatBalanceResult }) {
  const usesStream298 = result.chemicalHeatCalculationBasis === 'stream298'
  if (usesStream298) {
  ...
  useEffect(() => {
    setAssistantSnapshot((prev) => ({
      ...(prev ?? {
        ...
        selectedMethod: null,
        activeSheet: 'sb_smelting' as const,
        materialCount: 0,
        mixTotalWeight: null,
        totalCostPerHour: 0,
        materialsPreview: [],
...
export function AntimonyHeatBalancePlaceholderTables({
  darkMode,
  result,
  concentrateMassTh = 0,
  productResult,
