...
import { calculateGasMixtureStandardVolumeNm3h, calculateGasVolumePercents } from '../../../../utils/antimonyProductPhaseCalc'
import {
  calculateKnownTotal,
  materialWaterWeight,
...
import { AntimonyMaterialSelect } from './AntimonyMaterialSelect'
...
} from '../../../../utils/antimonyElementDisplay'
...
const GAS_ATTENTION_ELEMENT_KEYS = new Set<AntimonyElementKey>(['H(氢)', 'O(氧)', 'C (碳)', 'N(氮)'])
const WATER_ELEMENT_RATIOS = waterElementRatios()

function gasPhaseWeightPct(ratios: AntimonyRatios, waterWeightPct = 0) {
...
export function AntimonyBatchElementTable({
  darkMode,
  ...
  rawConcentrateRatios: Record<AntimonyElementKey, number>
  ...
  ratioInputValue: (kind: DraftRatioKind, id: string, element: AntimonyElementKey, fallback: number) => string | number
  rawWeightDrafts: Record<string, string>
  waterWeightDrafts: Record<string, string>
  ratioDrafts: Record<string, string>
  phaseCellStatus: (material: AntimonyMaterialColumn, element: AntimonyElementKey) => SolveInputStatus
  sulfurInputStatus: (ratios: AntimonyRatios) => SulfurInputStatus
  ...
  phaseUnknownElements: Set<AntimonyElementKey>
  ...
  onRawRatioChange: (id: string, element: AntimonyElementKey, value: string) => void
  onRawRatioBlur: (id: string, element: AntimonyElementKey, value: number | undefined) => void
  ...
  onSolventRatioChange: (id: string, element: AntimonyElementKey, value: string) => void
  onSolventRatioBlur: (id: string, element: AntimonyElementKey, value: number | undefined) => void
  onFuelRatioChange: (element: AntimonyElementKey, value: string) => void
  onFuelRatioBlur: (element: AntimonyElementKey, value: number | undefined) => void
  onGasRatioChange: (id: string, element: AntimonyElementKey, value: string) => void
  onGasRatioBlur: (id: string, element: AntimonyElementKey, value: number | undefined) => void
  ...
  const elementDisplayRatios = (ratios: Partial<Record<AntimonyElementKey, number>>) =>
    decomposeElementTableRatios(ratios, elementDisplayMode)
  const displayRatioValue = (ratios: Partial<Record<AntimonyElementKey, number>>, element: string) =>
  ...
  const elementAbsMinWidths = useMemo(() => {
    const collectSamples = (element: string) => {
      ...
      for (const material of rawMaterials) {
        samples.push(
          elementDisplayMode === 'compound'
            ? ratioInputValue('raw', material.id, element as AntimonyElementKey, material.ratios[element as AntimonyElementKey] ?? 0)
      ...
      for (const material of solventColumns) {
        samples.push(
          elementDisplayMode === 'compound'
            ? ratioInputValue('solvent', material.id, element as AntimonyElementKey, material.ratios[element as AntimonyElementKey] ?? 0)
            : formatTableNumber(displayRatioValue(material.ratios, element))
        )
      }
      samples.push(
        elementDisplayMode === 'compound'
          ? ratioInputValue('fuel', fuelColumn.id, element as AntimonyElementKey, fuelColumn.ratios[element as AntimonyElementKey] ?? 0)
      ...
      for (const material of airColumns) {
        samples.push(
          elementDisplayMode === 'compound'
            ? ratioInputValue('gas', material.id, element as AntimonyElementKey, material.ratios[element as AntimonyElementKey] ?? 0)
            : formatTableNumber(displayRatioValue(material.ratios, element))
        )
      }
  ...
  const waterWeightInputValue = (
    key: string,
    material: Pick<AntimonyMaterialColumn, 'weight' | 'waterWeight' | 'moisture' | 'kind' | 'airRole'>
  ) => {
  ...
  const renderTotalCell = (
    ...
    const total =
      elementDisplayMode === 'element'
        ? calculateElementTableDisplayTotal(ratios as Partial<Record<AntimonyElementKey, number>>, elementDisplayMode)
  ...
  const gasInputDisplayWeight = (column: AntimonyMaterialColumn) => {
        ...
        <tbody>
          ...
          {solventColumns.map((material, index) => (
            <Fragment key={material.id}>
              <tr>
                ...
                <td className={dataCellClass(darkMode, 'solvent')}>
                  <BatchTableNumericReadonly
                    ...
                    helpTitle="熔剂投料量由渣型及产出约束计算并自动回填。"
              ...
              {renderMaterialWaterRow(`${material.id}-water`, 'solvent', {
                waterWeightDisplay: materialWaterWeight(material),
                waterWeightInput: undefined,
                waterWeightHelpTitle: '熔剂含水质量随计算结果回填。',
              })}
            </Fragment>
          ))}
          {showFuel ? (
          <Fragment key="fuel-group">
            <tr>
              ...
              <td className={dataCellClass(darkMode, 'fuel')}>
                <BatchTableNumericReadonly
                  darkMode={darkMode}
                  className={readOnlyValueClass()}
                  helpTitle="燃料投料量由煤率约束计算并自动回填。"
                  value={fuelColumn.weight}
                />
              </td>
            ...
            {renderMaterialWaterRow('fuel-water', 'fuel', {
              waterWeightDisplay: materialWaterWeight(fuelColumn),
              waterWeightHelpTitle: '燃料含水质量随计算结果回填。',
            })}
          </Fragment>
          ) : null}
            ...
            <tr key={column.id}>
              ...
              <td className={dataCellClass(darkMode, 'oxygen')}>
                <BatchTableNumericReadonly
                  ...
                  helpTitle={`${column.name}投料量由供氧、富氧浓度及烟气含氧约束计算并自动回填（当前单位 ${gasInputUnitLabel}）。`}
                  className={readOnlyValueClass()}
                />
              </td>
                  ...
                  <tr key={rowKey}>
                    ...
                    {displayElementKeys.map((element) => (
                      ...
                      >
                        <BatchTableNumericReadonly
                          darkMode={darkMode}
                          value={displayRatioValue(product.composition as Partial<Record<AntimonyElementKey, number>>, element)}
                          className="text-sm"
                        />
                      </td>
                    ))}
                            ...
                            ? displayElementKeys.reduce(
                                (sum, element) =>
                                  ...
                                  Math.max(
                                    0,
                                    displayRatioValue(
                                      product.composition as Partial<Record<AntimonyElementKey, number>>,
                                      element
                                    )
                                  ),
                                0
