...
import { calculateGasMixtureStandardVolumeNm3h, PRODUCT_PHASE_ROWS } from '../../../../utils/antimonyProductPhaseCalc'
...
import type { AntimonyProductKey } from '../../../../utils/antimonyProcessCalc'
import { buildInputPhaseDisplayPlan } from '../../../../utils/antimonyInputPhaseTableDisplay'
...
function columnDisplayPhaseKeys(column: PhaseTableColumn, alwaysShowGasWater = false): string[] {
  ...
  if (column.kind === 'product') {
    ...
    if (column.productKey && column.productKey in PRODUCT_PHASE_ROWS) {
      return PRODUCT_PHASE_ROWS[column.productKey as AntimonyProductKey]
    }
    return []
  }
...
function isOutputPhaseRow(column: PhaseTableColumn, rowKey: string) {
  ...
  return PRODUCT_PHASE_ROWS[column.productKey as AntimonyProductKey].includes(rowKey)
}
...
export function AntimonyBatchPhaseTables({
  darkMode,
  phaseRowKeys,
  inputColumns,
  outputColumns,
